"""Asharq Al-Awsat Arabic scraper implementation."""
...
# e.g. /الاقتصاد/5309541-<slug> -- id starts right after the final "/".
ARTICLE_PATH_RE = re.compile(r"^/[^/]+/\d{6,}-")


class AawsatScraper(BaseScraper):
    """Scraper for Asharq Al-Awsat (aawsat.com).
    ...
    def __init__(self, settings: Settings | None = None):
        ...
        super().__init__(
            source_name="AsharqAlAwsat",
            base_url="https://aawsat.com",
            start_urls=app_settings.aawsat_seed_urls or [
    ...
    def is_valid_listing_url(self, listing_url: str) -> bool:
        ...
        return not parsed.netloc or parsed.netloc in ("aawsat.com", "www.aawsat.com")
    ...
    def parse_article(self, article_url: str, article_html: str) -> ArticleRecord | None:
        ...
        # First <h1> in document order is the real title; later ones belong
        # to "related articles" cards further down the page.
        title_node = soup.select_one("h1")
        title = normalize_whitespace(title_node.get_text(" ", strip=True)) if title_node else ""

        body_parts = [
            p.get_text(" ", strip=True)
            for p in soup.select("div.entry-content.first-article-node p")
        ]
        ...
        # data-publicationdate on the same container is a plain ISO string
        # and the most direct source; JSON-LD/meta are the fallbacks.
        published_date = None
        container = soup.select_one("div.entry-content.first-article-node")
        if container is not None:
            published_date = parse_datetime(container.get("data-publicationdate"))
