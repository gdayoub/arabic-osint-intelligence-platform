"""Al-Masry Al-Youm (Egypt) Arabic scraper implementation."""
...
# e.g. /news/details/4342514
ARTICLE_PATH_RE = re.compile(r"^/news/details/\d+")


class AlMasryAlYoumScraper(BaseScraper):
    """Scraper for Al-Masry Al-Youm Arabic (almasryalyoum.com).

    TODO(selector-maintenance): if the site updates layout,
    adjust selectors inside `extract_article_links` and `parse_article`.
    """

    NAME = "almasryalyoum_scraper"
    VERSION = "1.0.0"

    def __init__(self, settings: Settings | None = None):
        app_settings = settings or SETTINGS
        super().__init__(
            source_name="AlMasryAlYoum",
            base_url="https://www.almasryalyoum.com",
            start_urls=app_settings.almasryalyoum_seed_urls or ["https://www.almasryalyoum.com/"],
    ...
    def is_valid_listing_url(self, listing_url: str) -> bool:
        ...
        return not parsed.netloc or parsed.netloc in ("www.almasryalyoum.com", "almasryalyoum.com")
    ...
    def parse_article(self, article_url: str, article_html: str) -> ArticleRecord | None:
        ...
        # The real title h1 carries data-article_title too; either source
        # works, but the visible text is simplest and matches other scrapers.
        title_node = soup.select_one("h1.article-title")
        title = normalize_whitespace(title_node.get_text(" ", strip=True)) if title_node else ""

        # A same-titled <meta name="description"> duplicate of the lede
        # exists site-wide; the real body only lives under article-details.
        body_parts = [
            p.get_text(" ", strip=True)
            for p in soup.select("div.article-details p")
        ]
        body = normalize_whitespace(" ".join(body_parts))

        author_node = soup.select_one("[rel='author']") or soup.select_one(".writer-name")
